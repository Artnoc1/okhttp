...
internal class DerReader(source: Source) {
  ...
  fun readBitString(): Pair<ByteString, Int> {
    val buffer = Buffer()
    val trailingBits = readBitString(buffer)
    return buffer.readByteString() to trailingBits
  }

  fun readBitString(sink: Buffer): Int {
    if (bytesLeft != -1L) {
      val trailingBits = source.readByte().toInt() and 0xff
      source.read(sink, bytesLeft)
      return trailingBits
    } else {
      var trailingBits = 0
      readAll { tagClass, tag, constructed, length ->
        trailingBits = readBitString(sink)
      }
      return trailingBits
    }
  }
