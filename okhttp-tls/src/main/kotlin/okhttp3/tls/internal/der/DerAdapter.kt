...
internal abstract class DerAdapter<T>(
  ...
  companion object {
    ...
    /** The second element in the pair is the number of unused bits on the last byte. */
    val BIT_STRING = object : DerAdapter<Pair<ByteString, Int>>(
        tagClass = DerReader.TAG_CLASS_UNIVERSAL,
        tag = DerReader.TAG_BIT_STRING
    ) {
      override fun read(reader: DerReader): Pair<ByteString, Int> {
