...
class DerReaderTest {
  @Test fun `tag and length`() {
    ...
    assertThat(derReader.readValue { tagClass, tag, constructed, length ->
  ...
  @Test fun `primitive bit string`() {
    ...
    assertThat(derReader.readValue { tagClass, tag, constructed, length ->
      ...
      assertThat(derReader.readBitString()).isEqualTo("0A3B5F291CD0".decodeHex() to 4)
    }).isTrue()
